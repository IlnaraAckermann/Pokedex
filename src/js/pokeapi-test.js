...
function convertePokeApiDetailToPokemon(pokeDetail) {
  ...
  pokemon.stats = pokeDetail.stats.map((statName, statBase)=> [statName.stat.name, statBase.base_stat]);
  ...
  pokemon.evolution_chain = pokeDetail.chain.evolves_to.species.name
  return pokemon
}

pokeApi.getPokemonsEvolutionDetail = (pokemon) => {
